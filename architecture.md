...
| `/category-manager/promo-simulator`           | Promo simulator — default chrome (9-step wizard) | Stepper (vertical sticky right rail), StepContent dispatcher, Step1Brief (cascading Category→Segment→Product, auto-populated categoryManager, editable end-date picker, ArchiveSheet + BackgroundDataSheet side sheets), Step2–9, LiveKPIPanel, AINarrative (templated), PromoSummaryCard, AltDesignsStrip (links to variants) |
