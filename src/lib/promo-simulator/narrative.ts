...
const GOAL_NARRATIVE: Record<Goal, string[]> = {
  ...
  'אחר / חוצה קטגוריות': [
    'מבצעי צלב-קטגוריה עובדים מצוין לאירועי שיא (חגים, עונות). המשותף שלהם הוא נושא ברור שהקונה מזהה מיד.',
    'שילוב שתי קטגוריות משלימות ברשימה אחת מעלה את שיעור ההצלבה ואת ערך העסקה.',
  ],
}
...
export function narrativeFor(state: SimulatorState): string[] {
  switch (state.step) {
    case 2:
      return state.goal ? GOAL_NARRATIVE[state.goal] : [
