...
export function calcMetrics(state: SimulatorState): PromoMetrics {
  ...
  const baseProfit = Math.round(state.baseUnits * Math.max(state.unitPrice - state.unitCost, 0))
  ...
  const roi =
    investment > 0 ? Math.round(((promoProfit - baseProfit - investment) / investment) * 100) : 0
  const breakEvenUnits =
    unitMargin > 0 ? Math.ceil(investment / unitMargin) : Number.POSITIVE_INFINITY
  const stockCoverage =
...
/**
 * Returns a performance ratio (0..1+) suitable for feeding into getKpiStatusColor.
 * worthIt → 1 (green), needsImprovement → 0.9 (amber), notWorthIt → 0.5 (red).
 */
export function statusRatio(status: PromoStatus): number {
  switch (status) {
    case 'worthIt':
      return 1
    case 'needsImprovement':
