...
- הצע פעולות קונקרטיות שמנהל סחר צריך לנקוט (לשפר רווח גולמי באחוז נדרש ( באחוז שהוא פער מהיעד), שינוי מיקס מבצעים, הרחבת קטגוריה, וכו׳)
- נושא = תחום אסטרטגי (לדוגמה: רווח גולמי, אפקטיביות מבצעים, חוסרים במדף, צמיחת קטגוריה)
- המלצה = פעולה ספציפית עם מספרים ופרטים
- ודא שיש מגוון סטטוסים (לא הכל אדום או ירוק)
...
function buildChainPayload({
  ...
  const supplierSummary = suppliers.slice(0, 12).map((s) => ({
    name: s.name,
    sales: formatCurrencyShort(s.sales * actualMultiplier),
    targetAchievement:
      s.targetSales > 0
        ? `${((s.sales / s.targetSales) * 100).toFixed(1)}%`
        : "N/A",
