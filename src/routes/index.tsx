...
import {
  BarChart3, ShieldCheck, Users, MapPin, Bell, Cpu, Sparkles, ArrowLeft, ChevronDown,
} from 'lucide-react'
...
const features = [
  { icon: BarChart3, title: 'ניתוח מכירות', description: 'מעקב מכירות בזמן אמת עם השוואות בין סניפים ומגמות צמיחה', color: '#DC4E59', span: 'lg:col-span-2' },
  { icon: Sparkles, title: 'AI תובנות', description: 'ניתוח אוטומטי עם בינה מלאכותית — תדריך בוקר והמלצות פעולה', color: '#6C5CE7', span: '' },
  { icon: Users, title: 'ניהול כח אדם', description: 'תמונת מצב תקינה, תחלופה ועמידה ביעדי שעות', color: '#2EC4D5', span: '' },
  { icon: ShieldCheck, title: 'בקרת איכות ומלאי', description: 'ניטור ציוני איכות, ימי מלאי וזיהוי חריגות אוטומטי', color: '#F6B93B', span: 'lg:col-span-2' },
  { icon: MapPin, title: 'מפת סניפים', description: 'מפה אינטראקטיבית עם סטטוס ביצועים ודירוג אזורי', color: '#DC4E59', span: '' },
  { icon: Bell, title: 'התראות חכמות', description: 'התראות על חריגות, יעדים ואירועים חריגים בזמן אמת', color: '#2EC4D5', span: '' },
  { icon: Cpu, title: 'דוחות מותאמים', description: 'דוחות מפורטים לפי תפקיד עם סינון, ייצוא והשוואה', color: '#6C5CE7', span: 'lg:col-span-2' },
]
  ...
  return (
    <section ref={ref} className="relative min-h-[85vh] flex items-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full opacity-[0.07]" style={{ background: 'radial-gradient(circle, #DC4E59 0%, transparent 70%)' }} />
        <div className="absolute top-1/3 -right-20 w-72 h-72 rounded-full opacity-[0.05]" style={{ background: 'radial-gradient(circle, #6C5CE7 0%, transparent 70%)' }} />
        <div className="absolute bottom-20 left-1/4 w-48 h-48 rounded-full opacity-[0.06]" style={{ background: 'radial-gradient(circle, #2EC4D5 0%, transparent 70%)' }} />
      </div>
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center px-4 sm:px-6 py-10">
        ...
        >
          ...
          <motion.p
            ...
            style={{ color: '#4A5568' }}
          >
            פלטפורמת ניתוח וניהול שמרכזת את כל הנתונים של הרשת במקום אחד ומספקת תובנות ממוקדות לכל דרג ניהולי.
          </motion.p>
          ...
          >
            {stats.map(stat => (
              <div key={stat.label} className="text-center">
                <p className="text-2xl font-bold font-mono" style={{ color: '#DC4E59' }} dir="ltr">{stat.value}</p>
                <p className="text-xs mt-0.5" style={{ color: '#A0AEC0' }}>{stat.label}</p>
              </div>
            ))}
          </motion.div>
        </motion.div>
        ...
        >
          ...
          >
            ...
            <p className="text-lg font-bold font-mono text-[#2D3748]" dir="ltr">₪847K</p>
            <span className="text-[10px] font-semibold text-[#2EC4D5]" dir="ltr">▲ 12.3%</span>
          </motion.div>
          ...
          >
            ...
            <p className="text-lg font-bold font-mono text-[#6C5CE7]" dir="ltr">92</p>
            <span className="text-[10px] font-semibold text-[#2EC4D5]" dir="ltr">▲ 8pts</span>
          </motion.div>
          ...
          >
            <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ background: '#6C5CE7' + '20' }}>
              <Sparkles className="w-3 h-3 text-[#6C5CE7]" />
            </div>
            <p className="text-[10px] text-[#4A5568] font-medium">AI זיהה 3 חריגות</p>
          </motion.div>
        </motion.div>
      </div>
...
function FeaturesSection() {
  return (
    <section className="py-20 lg:py-28 px-4 sm:px-6">
      ...
      >
        <h2 className="text-3xl lg:text-4xl font-bold mb-4" style={{ color: '#2D3748' }}>
          הכל במקום אחד
        </h2>
        ...
        {features.map((feature, i) => (
          ...
          >
            ...
            <div className="relative space-y-3">
              ...
              <h3 className="text-base font-bold" style={{ color: '#2D3748' }}>{feature.title}</h3>
              <p className="text-sm leading-relaxed" style={{ color: '#4A5568' }}>{feature.description}</p>
            </div>
          </motion.div>
        ))}
      </div>
  ...
  return (
    <section className="py-20 px-4 sm:px-6">
      ...
      >
        ...
        <div className="absolute top-0 right-0 w-60 h-60 rounded-full opacity-20 blur-3xl" style={{ background: '#DC4E59' }} />
        <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full opacity-15 blur-3xl" style={{ background: '#6C5CE7' }} />
        ...
        >
          {[...Array(2)].map((_, setIdx) => (
            <div key={setIdx} className="flex gap-12 items-center">
              {['ניתוח מכירות', 'בקרת איכות', 'כח אדם', 'ניהול מלאי', 'AI תובנות', 'מפת סניפים', 'דוחות', 'התראות'].map(text => (
                <span key={`${setIdx}-${text}`} className="text-sm font-medium" style={{ color: '#A0AEC0' }}>
