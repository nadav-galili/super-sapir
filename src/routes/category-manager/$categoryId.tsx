...
const SEASONAL_WEIGHTS = [
  0.88, 0.85, 0.95, 1.08, 1.02, 1.05, 1.04, 1.0, 1.1, 1.06, 0.98, 0.99,
];
...
function CategoryDrillDown() {
  ...
  const derived = useMemo(() => {
    ...
    for (let bi = 0; bi < allBranches.length; bi++) {
      ...
      rows.push({
        ...
        targetAchievement:
          adjustedTarget > 0 ? (dept.sales / adjustedTarget) * 100 : 100,
    ...
    const targetScale =
      totalMonthTarget > 0 ? totalMonthSales / totalMonthTarget : 1;
    ...
    for (const row of rows) {
      ...
      if (row.grossMarginPercent < networkAvgMargin - 3) {
        alerts.push({
          ...
          kind: "margin-drop",
          severity:
            row.grossMarginPercent < networkAvgMargin - 5
              ? "critical"
              : "warning",
    ...
    return {
      ...
      alerts,
      groupedAlerts: Array.from(groupedAlerts.entries()),
    };
  }, [categoryId]);
  ...
  if (branchData.length === 0) {
    return (
      <PageContainer>
        <Breadcrumbs
          items={[{ label: "מנהל קטגוריה", to: "/category-manager" }]}
        />
        <div className="text-center py-16">
          ...
          <p className="text-[#4A5568] mt-2">
            הקטגוריה &quot;{categoryId}&quot; לא קיימת במערכת
          </p>
        </div>
      </PageContainer>
    );
        ...
        <Card className="border-warm-border rounded-[16px]">
          <CardHeader className="pb-2">
            <CardTitle className="text-xl text-[#2D3748]">
              מגמת מכירות — {categoryName} (12 חודשים)
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <table className="w-full min-w-[500px] text-lg">
                  ...
                  <tbody>
                    {groupedAlerts.map(([branch, branchAlerts], i) => {
                      const hasCritical = branchAlerts.some(
