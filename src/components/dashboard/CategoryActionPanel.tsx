...
export function CategoryActionPanel({
  title,
  tone,
  items,
}: CategoryActionPanelProps) {
  const Icon = tone === "danger" ? TriangleAlert : ArrowUpRight;
  const accentClass = tone === "danger" ? "text-[#DC4E59]" : "text-[#2EC4D5]";
    ...
    <Card className="h-full">
      ...
      <CardContent className="space-y-3">
        {items.map((item, index) => {
          const impact =
            tone === "danger" ? item.downsideEstimate : item.upsideEstimate;
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                  <div className="space-y-1.5 sm:space-y-2 min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-[#788390]">
                      {item.focusAction}
                    </p>
                  </div>
                  ...
                  <div className="text-right sm:text-left shrink-0" dir="ltr">
                    <p
                      className={`text-base sm:text-lg font-bold font-mono ${accentClass}`}
                    >
                      {formatCurrencyShort(impact)}
                    </p>
                    <p className="text-[11px] text-[#788390]">{label}</p>
                  </div>
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-[#4A5568]">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {item.comparisonChange}% {item.comparisonLabel}
                  </span>
                  <span className="inline-flex items-center gap-1">
                    <Store className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#2EC4D5] shrink-0" />
                    <span className="truncate">
                      מוביל: {item.topBranchName} | חלש:{" "}
                      {item.weakestBranchName}
                    </span>
                  </span>
                </div>
              </Link>
