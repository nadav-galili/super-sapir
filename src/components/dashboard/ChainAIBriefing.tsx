...
import { useChainAIAnalysis } from '@/hooks/useChainAIAnalysis'
import type { ChainInsightRow } from '@/lib/chain-ai'

const STATUS_CONFIG: Record<ChainInsightRow['status'], { label: string; color: string; bg: string }> = {
...
export function ChainAIBriefing() {
  const { rows, isLoading, isStreaming, error, retry } = useChainAIAnalysis()
      ...
      <Card className="border-warm-border rounded-[16px] overflow-hidden relative">
        ...
        <CardHeader className="pb-2">
          <CardTitle className="text-xl flex items-center gap-2 text-[#2D3748]">
            ...
            {isStreaming && (
              <span className="text-[15px] text-[#6C5CE7] font-medium animate-pulse me-auto">מנתח נתוני רשת...</span>
            )}
          </CardTitle>
        </CardHeader>
