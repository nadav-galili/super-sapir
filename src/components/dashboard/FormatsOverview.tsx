  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-[16px] text-[#4A5568] font-medium">{kpi.label}</span>
        <div className="flex items-center gap-2">
          <span className="text-lg font-bold font-mono text-[#2D3748]" dir="ltr">{displayValue}</span>
          <span
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <span className="text-[16px] text-[#A0AEC0] font-medium">{branchCount} סניפים</span>
      </div>
...
export function FormatsOverview() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <FormatCard title="חנויות גדולות" branchCount={bigBranches.length} kpis={bigKpis} delay={0.1} />
      <FormatCard title="חנויות עיר" branchCount={cityBranches.length} kpis={cityKpis} delay={0.2} />
    </div>
  )
}
