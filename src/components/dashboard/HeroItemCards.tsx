      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-base font-bold text-[#2D3748] truncate">{productName}</p>
            <p className="text-sm text-[#A0AEC0] mt-0.5">{categoryName}</p>
            <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-2.5">
              {stats.map(s => (
                <div key={s.label}>
                  <p className="text-[11px] text-[#A0AEC0]">{s.label}</p>
                  <p className="text-sm font-bold font-mono" style={{ color: s.color }} dir="ltr">
                    {s.value}
                  </p>
                </div>
              ))}
