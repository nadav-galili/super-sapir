...
import { getCategorySuppliers, type CategorySupplier } from '@/data/mock-category-suppliers'
...
function SupplierKpiCard({ title, icon, iconBg, accentColor, name, stats, delay }: {
  title: string
...
export function CategorySuppliersDashboard({ categoryId, categoryName }: CategorySuppliersDashboardProps) {
  const suppliers = useMemo(() => getCategorySuppliers(categoryId), [categoryId])

  const { bestMargin, worstTarget, highStockout } = useMemo(() => {
    if (suppliers.length === 0) return { bestMargin: null, worstTarget: null, highStockout: null }
    const bm = suppliers.reduce((best, s) => s.grossProfitPercent > best.grossProfitPercent ? s : best)
    ...
    const hs = suppliers.reduce((worst, s) => s.stockoutRate > worst.stockoutRate ? s : worst)
    return { bestMargin: bm, worstTarget: wt, highStockout: hs }
  }, [suppliers])

  const { chartData, maxSales } = useMemo(() => {
    const cd = suppliers.map(s => ({ name: s.name, sales: s.sales, target: s.targetSales }))
    let max = 0
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {bestMargin && (
          <SupplierKpiCard
            ...
            delay={0.1}
            stats={[
              { label: 'רווח גולמי', value: `${bestMargin.grossProfitPercent}%`, color: '#6C5CE7' },
              { label: 'מכירות', value: formatCurrencyShort(bestMargin.sales), color: '#2D3748' },
            ]}
          />
        )}
        {worstTarget && (
          <SupplierKpiCard
            ...
            delay={0.15}
            stats={[
              { label: 'עמידה ביעד', value: `${(worstTarget.targetSales > 0 ? (worstTarget.sales / worstTarget.targetSales) * 100 : 100).toFixed(1)}%`, color: '#DC4E59' },
              { label: 'פער', value: formatCurrencyShort(worstTarget.targetSales - worstTarget.sales), color: '#DC4E59' },
            ]}
          />
        )}
        {highStockout && (
          <SupplierKpiCard
            ...
            delay={0.2}
            stats={[
              { label: 'שיעור חוסרים', value: `${highStockout.stockoutRate}%`, color: '#F6B93B' },
              { label: 'פריטים', value: String(highStockout.productCount), color: '#2D3748' },
            ]}
          />
        )}
      </div>
              ...
              <div dir="ltr" className="h-[260px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barGap={2}>
                    ...
                    <Bar dataKey="target" fill="#A0AEC0" opacity={0.3} radius={[4, 4, 0, 0]} animationDuration={1000} animationBegin={200} barSize={18} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#FFF0EA]">
                      ...
                      <th className="px-3 py-2 text-right font-medium text-[#A0AEC0]">יעד</th>
                      <th className="px-3 py-2 text-right font-medium text-[#A0AEC0]">רווח %</th>
                      <th className="px-3 py-2 text-right font-medium text-[#A0AEC0]">חוסרים</th>
                      <th className="px-3 py-2 text-right font-medium text-[#A0AEC0]">פריטים</th>
                    </tr>
                  </thead>
                  <tbody>
                    {suppliers.map((sup, i) => {
                      ...
                      const barPct = maxSales > 0 ? (sup.sales / maxSales) * 100 : 0
