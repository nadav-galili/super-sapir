...
import {
  CATEGORY_MANAGER_INVENTORY_DAYS_GOAL,
  CATEGORY_MANAGER_MARGIN_BASELINE,
  getComparisonLabel,
  type CategorySnapshot,
} from "@/lib/category-manager";
...
function getInventoryDaysColor(inventoryDays: number): string {
  ...
  if (inventoryDays >= CATEGORY_MANAGER_INVENTORY_DAYS_GOAL - 1)
    return "#F6B93B";
  return "#2EC4D5";
}
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="space-y-1">
            ...
            <p className="text-xs text-[#788390]">{row.original.focusAction}</p>
          </div>
        ),
      },
      {
        ...
        cell: ({ getValue }) => (
          <span
            ...
            dir="ltr"
            style={{
              color: (getValue() as number) >= 0 ? "#2EC4D5" : "#DC4E59",
            }}
          >
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <span
              className="font-mono"
              dir="ltr"
              style={{ color: val >= 0 ? "#2EC4D5" : "#DC4E59" }}
            >
              {val > 0 ? "+" : ""}
              {val.toFixed(1)}%
            </span>
      ...
      },
      {
        ...
        header: ({ column }) => (
          <SortHeader column={column} label="רווח גולמי" />
        ),
        cell: ({ getValue }) => {
          const val = getValue() as number;
          return (
            <span
              ...
              style={{
                color:
                  val >= CATEGORY_MANAGER_MARGIN_BASELINE
                    ? "#2D3748"
                    : "#DC4E59",
              }}
            >
        ...
        cell: ({ getValue }) => {
          ...
          return (
            <div className="space-y-0.5">
              ...
              <p className="whitespace-nowrap text-[11px] text-[#788390]">
                {delta > 0
                  ? `${delta}+ מעל היעד`
                  : delta < 0
                    ? `${Math.abs(delta)} מתחת ליעד`
                    : "בדיוק ביעד"}
              </p>
            </div>
          );
      ...
      },
      {
        ...
        header: ({ column }) => (
          <SortHeader column={column} label="פדיון בסיכון" />
        ),
        cell: ({ getValue }) => (
      ...
      },
      {
        ...
        header: ({ column }) => (
          <SortHeader column={column} label="ROI מבצעים" />
        ),
        cell: ({ row, getValue }) => {
          ...
          return (
            <div className="space-y-0.5 whitespace-nowrap">
              <p>
                <span className="font-mono font-semibold text-[#2EC4D5]">
                  {value.toFixed(2)}x
                </span>
              </p>
              <p className="text-[11px] text-[#788390]">
                uplift {row.original.avgPromoUplift.toFixed(1)}%
              </p>
            </div>
          );
        },
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="space-y-0.5 whitespace-nowrap">
              ...
              <p className="text-[11px] text-[#788390]">
                {row.original.status === "opportunity" ? "פוטנציאל" : "סיכון"}{" "}
                מוערך
              </p>
            </div>
          );
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="text-lg text-[#2D3748]">
                לוח פעולה לפי קטגוריה
              </CardTitle>
              <p className="mt-1 text-xs sm:text-sm text-[#4A5568]">
                השוואה פעילה: {comparisonLabel} | יעד ימי מלאי:{" "}
                {CATEGORY_MANAGER_INVENTORY_DAYS_GOAL}
              </p>
            </div>
            <ComparisonToggle
                ...
                {table.getHeaderGroups().map((hg) => (
                  <tr key={hg.id} className="border-b border-[#FFF0EA]">
                    {hg.headers.map((header) => (
                      <th
                        key={header.id}
                        className="px-3 py-2 text-right font-medium text-[#788390]"
                      >
