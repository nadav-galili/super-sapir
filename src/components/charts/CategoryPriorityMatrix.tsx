...
function getNearbyPoints(points: MatrixChartPoint[], point: MatrixChartPoint) {
  return points.filter(
    (candidate) =>
      ...
      Math.abs(
        candidate.normalizedGrossMarginPercent -
          point.normalizedGrossMarginPercent
      ) <= 1.2
  );
}

function getConflictingPoints(
  points: MatrixChartPoint[],
  point: MatrixChartPoint
) {
  return points.filter(
    (candidate) =>
      candidate.category.id !== point.category.id &&
...
function buildChartData(data: CategorySnapshot[]): MatrixChartPoint[] {
  ...
  const sortedPoints = [...basePoints].sort((a, b) => {
    if (a.comparisonChange !== b.comparisonChange)
      return a.comparisonChange - b.comparisonChange;
  ...
  for (const point of sortedPoints) {
    ...
    const usedOffsets = new Set(
      nearbyPoints
        .filter(
          (candidate) =>
            candidate.plotX !== candidate.comparisonChange ||
            candidate.plotY !== candidate.normalizedGrossMarginPercent
        )
        .map(
          (candidate) =>
            `${(candidate.plotX - candidate.comparisonChange).toFixed(2)}:${(candidate.plotY - candidate.normalizedGrossMarginPercent).toFixed(2)}`
        )
    );

    const preferredOffset =
      BUBBLE_OFFSETS.find((offset) => {
    ...
    point.plotY = +(
      point.normalizedGrossMarginPercent + preferredOffset.dy
    ).toFixed(2);
  }
  ...
  for (const point of sortedPoints) {
    ...
    const usedPlacements = new Set(
      conflictingPoints
        .filter(
          (candidate) => candidate.labelDy !== -18 || candidate.labelDx !== 0
        )
        .map(
          (candidate) =>
            `${candidate.labelDx}:${candidate.labelDy}:${candidate.labelAnchor}`
        )
    );

    const preferredPlacement =
      LABEL_PLACEMENTS.find((placement) => {
...
function MatrixTooltip({ active, payload }: MatrixTooltipProps) {
  ...
  return (
    <div
      className="rounded-[12px] border border-[#FFE8DE] bg-white p-3 shadow-sm"
      dir="rtl"
    >
      ...
      <p className="text-sm text-[#4A5568]">
        ...
        <span className="font-mono" dir="ltr">
          {item.comparisonChange > 0 ? "+" : ""}
          {item.comparisonChange.toFixed(1)}%
        </span>
      </p>
      <p className="text-sm text-[#4A5568]">
        רווח גולמי:{" "}
        <span className="font-mono" dir="ltr">
          {item.normalizedGrossMarginPercent.toFixed(1)}%
        </span>
      </p>
      <p className="mt-1 text-xs text-[#788390]">{item.focusAction}</p>
    </div>
  );
}
...
export function CategoryPriorityMatrix({ data }: CategoryPriorityMatrixProps) {
  const averageMargin =
    data.length > 0
      ? data.reduce((sum, item) => sum + item.normalizedGrossMarginPercent, 0) /
        data.length
      : 0;
      ...
      <Card className="h-full">
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-[#4A5568]">
              <span className="rounded-full bg-[#DC4E59]/10 px-2.5 py-1 text-[#DC4E59]">
                סיכון
              </span>
              <span className="rounded-full bg-[#2EC4D5]/10 px-2.5 py-1 text-[#2EC4D5]">
                הזדמנות
              </span>
              <span className="rounded-full bg-[#F6B93B]/10 px-2.5 py-1 text-[#F6B93B]">
                מעקב
              </span>
            </div>
          </div>
        ...
        <CardContent>
          <div className="relative">
            <div
              className="pointer-events-none absolute inset-x-6 top-2 z-10 hidden justify-between text-xs text-[#788390] md:flex"
              dir="ltr"
            >
              <span>לשקם</span>
              <span>להאיץ</span>
            ...
            <div
              className="pointer-events-none absolute inset-x-6 bottom-6 z-10 hidden justify-between text-xs text-[#788390] md:flex"
              dir="ltr"
            >
              <span>סיכון</span>
            ...
            <div dir="ltr" className="h-[300px] sm:h-[420px]">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart
                  margin={{ top: 40, right: 12, bottom: 12, left: 4 }}
                >
                  ...
                  <ReferenceLine x={0} stroke="#788390" strokeDasharray="4 4" />
                  <ReferenceLine
                    y={averageMargin}
                    stroke="#788390"
                    strokeDasharray="4 4"
                      ...
                      return (
                        <g>
                          ...
                          <text
                            ...
                            direction="rtl"
                          >
                            {payload.labelLines.map(
                              (line: string, index: number) => (
                                <tspan
                                  key={`${payload.category.id}-${line}`}
                                  x={labelX}
                                  dy={index === 0 ? 0 : 12}
                                >
                                  {line}
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            {chartData.slice(0, 3).map((item) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-[#788390]">
                  {item.focusAction}
                </p>
              </Link>
            ))}
