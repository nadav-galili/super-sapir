...
describe('usePromoSimulator (boundary contract)', () => {
  ...
  describe('state → narrative', () => {
    it('produces goal-specific copy at step 2', () => {
      const state: SimulatorState = { ...defaults, step: 2, goal: 'משיכת קונים' }
  ...
  describe('step-jump validation', () => {
    ...
    it('allows forward jumps (soft-warning model — UI handles gaps)', () => {
      const state: SimulatorState = { ...defaults, step: 2 }
